...
float4 evaluate_effect(EffectQuad fx, float2 position) {
  ...
  // Per-tile domain: offset, scale, and flow heading from the seed so
  ...
  if (progress > 0.0) {
    ...
    float band = smoothstep(progress - 0.22, progress, x) *
                 (1.0 - smoothstep(progress, progress + 0.26, x));
    field = max(field, band * 0.8);
  }

  // Center a lattice that covers the rect; overflow is clipped by the
  // rounded SDF so edge dots sit against the tile, not a half-cell inset.
  ...
  float in_grid = step(0.0, id.x) * step(0.0, id.y) *
                  step(id.x, count.x - 1.0) * step(id.y, count.y - 1.0);
